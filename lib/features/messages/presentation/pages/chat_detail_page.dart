...
class _ChatDetailPageState extends ConsumerState<ChatDetailPage> {
  ...
  static final Color _myMessageColor = AppColors.primary;
  static final Color _otherMessageColor = AppColors.surfaceVariant;
  static final Color _reactionBgColor = AppColors.divider;
  ...
  Future<void> _loadCurrentProfile() async {
    ...
    if (mounted) {
      setState(() {
        _currentProfileId = userDoc.data()?['activeProfileId'] as String? ?? currentUser.uid;
      });
    }
  }
    ...
    _messagesSubscription = FirebaseFirestore.instance
      ...
      final messages = snapshot.docs.map((doc) {
        ...
        return {
          ...
          'senderProfileId': data['senderProfileId'] ?? data['senderId'] ?? '', // Fallback para mensagens antigas
          'text': data['text'] ?? '',
  ...
  Future<void> _markConversationAsRead() async {
    ...
    try {
      ...
      await ref.read(messageServiceProvider).markAsRead(widget.conversationId, currentProfileId);
    } catch (e) {
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      ...
      // ✅ FIX: Usar ref.read ao invés de instância manual
      await ref.read(notificationServiceProvider).createNewMessageNotification(
        conversationId: widget.conversationId,
        recipientProfileId: widget.otherProfileId,
                ...
                StreamBuilder<DocumentSnapshot>(
                  ...
                  builder: (context, snapshot) {
                    final isOnline = snapshot.hasData &&
                        ...
                        (snapshot.data!.data() as Map?)?.containsKey('isOnline') == true &&
  ...
  Widget _buildMessageBubble(Map<String, dynamic> message) {
    final currentUser = FirebaseAuth.instance.currentUser;
    final isMyMessage = message['senderProfileId'] == _currentProfileId; // Usa profileId
                      ...
                      children: [
                              ...
                              children: [
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      Text(
                                        replyTo['senderProfileId'] == _currentProfileId
                                            ? 'Você'
                                            : widget.otherUserName,
                                        style: TextStyle(
                                          fontSize: 12,
                        ...
                        if (message['text'].toString().isNotEmpty)
                          Padding(
                            ...
                            child: Linkify(
                              onOpen: (link) async {
                                // Tentar processar como deep link do app primeiro
                                final handled = await DeepLinkHandler.handleLink(
                                  context,
                                  link.url,
                                );
                                
                                // Se não for link do app, abrir no navegador
                                if (!handled) {
