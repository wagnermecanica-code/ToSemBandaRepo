...
class MessagesRemoteDataSource implements IMessagesRemoteDataSource {
  ...
  Future<List<ConversationEntity>> getConversations({
    ...
    try {
      debugPrint('🔍 MessagesDataSource: getConversations - profileId=$profileId');
      ...
      if (startAfter != null) {
        query = query.startAfter([Timestamp.fromDate(startAfter.lastMessageTimestamp)]);
      }
      ...
      debugPrint('✅ MessagesDataSource: ${conversations.length} conversas carregadas');
  ...
  Future<ConversationEntity> getOrCreateConversation({
    ...
    try {
      ...
      final snapshot = await _firestore
          .collection('conversations')
          .where('participantProfiles', arrayContainsAny: [currentProfileId, otherProfileId])
          .get();
  ...
  Future<List<MessageEntity>> getMessages({
    ...
    try {
      debugPrint('🔍 MessagesDataSource: getMessages - conversationId=$conversationId');
      ...
      debugPrint('✅ MessagesDataSource: ${messages.length} mensagens carregadas');
      return messages;
    } catch (e) {
  ...
  Future<void> markAsRead(String conversationId, String profileId) async {
    try {
      debugPrint('✅ MessagesDataSource: markAsRead - conversationId=$conversationId');
      await _firestore.collection('conversations').doc(conversationId).update({
  ...
  Future<void> markAsUnread(String conversationId, String profileId) async {
    try {
      debugPrint('📬 MessagesDataSource: markAsUnread - conversationId=$conversationId');
      await _firestore.collection('conversations').doc(conversationId).update({
  ...
  Future<void> deleteConversation(String conversationId, String profileId) async {
    try {
      debugPrint('🗑️ MessagesDataSource: deleteConversation - conversationId=$conversationId');
