...
import '../../../../../packages/core_ui/lib/widgets/multi_select_field.dart';
import '../../../../../packages/core_ui/lib/theme/app_colors.dart';
import 'package:image_picker/image_picker.dart';
import 'package:image_cropper/image_cropper.dart';
import 'package:flutter_image_compress/flutter_image_compress.dart';
import 'package:path/path.dart' as p;
import 'package:flutter_riverpod/flutter_riverpod.dart';
import '../../../../../packages/core_ui/lib/di/profile_providers.dart';
import '../../../auth/presentation/providers/auth_providers.dart';
import '../../../../../packages/core_ui/lib/profile_result.dart';
import '../../domain/entities/profile_entity.dart';
import '../../../../../packages/core_ui/lib/navigation/bottom_nav_scaffold.dart';
...
class _EditProfilePageState extends ConsumerState<EditProfilePage> {
  ...
  static const List<String> _instrumentOptions = [
    'Violão', 'Guitarra', 'Baixo', 'Contrabaixo', 'Bateria', 'Teclado',
    'Piano', 'Saxofone', 'Flauta', 'Trompete', 'Violino', 'Cello',
    'Voz (cantor)', 'DJ', 'Percussão', 'Harmônica', 'Ukulele',
  ];

  static const List<String> _genreOptions = [
    'Rock', 'Pop', 'Jazz', 'Blues', 'Funk', 'Soul', 'R&B', 'Reggae',
    'MPB', 'Sertanejo', 'Forró', 'Axé', 'Hip-Hop', 'Rap', 'Eletrônica',
    'Folk', 'Country', 'Classical', 'Metal', 'Punk', 'Indie', 'Samba',
    'Bossa Nova', 'Gospel',
  ];
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      if (widget.profileIdToEdit != null) {
        debugPrint('EditProfile: Carregando perfil específico: ${widget.profileIdToEdit}');
        ...
        if (doc.exists) {
          final ProfileEntity profile = ProfileEntity.fromFirestore(doc);
          _profile = profile;
        ...
        if (activeProfile != null) {
          ...
          final parts = <String>[];
          if (activeProfile.neighborhood != null && activeProfile.neighborhood!.isNotEmpty) {
  ...
  void _onAddressSelected(Map<String, dynamic> suggestion) {
    final double lat = double.tryParse((suggestion['lat'] as String?) ?? '') ?? 0.0;
    final double lon = double.tryParse((suggestion['lon'] as String?) ?? '') ?? 0.0;
    
    if (lat != 0.0 && lon != 0.0) {
      final Map<String, dynamic>? address = suggestion['address'] as Map<String, dynamic>?;
      
      // Extrair componentes do endereço
      final String neighbourhood = (address?['neighbourhood'] as String?) ?? 
      ...
      setState(() {
        ...
        _locationController.text = parts.isNotEmpty ? parts.join(', ') : (suggestion['display_name'] as String?) ?? '';
        _selectedCity = city;
  ...
  Future<void> _pickAndCropProfileImage() async {
    try {
      ...
      if (compressed != null && mounted) {
        ...
        debugPrint('EditProfile: Imagem comprimida com sucesso: $compressedPath');
        setState(() {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Foto selecionada! Clique em "Salvar Alterações" para confirmar.'),
        ...
        debugPrint('EditProfile: Compressão retornou null ou widget não montado');
      }
    } catch (e) {
  ...
  Future<void> _saveProfile() async {
    ...
    if (_isBand == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Por favor, selecione o tipo de perfil (Músico ou Banda)'),
    ...
    try {
      ...
      if (widget.isNewProfile || currentProfile == null) {
        ...
        if (_selectedLocation == null || _selectedCity == null || _selectedCity!.isEmpty) {
        ...
        final newProfile = Profile(
          ...
          notificationRadius: 20.0,
          notificationRadiusEnabled: true,
        );
        
        // ✅ Usar profileProvider.notifier (Clean Architecture)
        final result = await ref.read(profileProvider.notifier).createProfile(newProfile);
        
        switch (result) {
          case ProfileSuccess(:final profile):
            debugPrint('✅ EditProfile: Perfil criado - ID=${profile.profileId}');
            
            if (mounted) {
              // ✅ Definir manualmente o activeProfileId no users doc
              final user = FirebaseAuth.instance.currentUser;
              if (user != null) {
                debugPrint('🔄 EditProfile: Atualizando activeProfileId para ${profile.profileId}...');
                await FirebaseFirestore.instance
                    .collection('users')
                    .doc(user.uid)
                    .set({'activeProfileId': profile.profileId}, SetOptions(merge: true));
                
                debugPrint('✅ EditProfile: activeProfileId atualizado no Firestore');
                ...
                if (profileState.hasValue) {
                  debugPrint('✅ EditProfile: Perfil ativo atual: ${profileState.value?.activeProfile?.name} (${profileState.value?.activeProfile?.profileId})');
                }
              }
              ...
              if (mounted && context.mounted) {
                ...
                if (widget.isNewProfile && widget.profileIdToEdit == null) {
                  debugPrint('🏠 EditProfile: Primeiro acesso - redirecionando para home');
        ...
        debugPrint('🔄 EditProfile: Atualizando perfil existente - ID=${currentProfile.profileId}');
        ...
        // ✅ Usar profileProvider.notifier (Clean Architecture)
        final result = await ref.read(profileProvider.notifier).updateProfile(updatedProfile);
        
        switch (result) {
          case ProfileSuccess(:final profile):
            debugPrint('✅ EditProfile: Perfil atualizado - ID=${profile.profileId}');
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(
            widget.isNewProfile ? 'Novo Perfil' : (_isFirstAccess ? 'Complete seu Perfil' : 'Editar Perfil'),
      ...
      children: [
        ...
        Center(
          child: GestureDetector(
            onTap: () {
              debugPrint('EditProfile: CircleAvatar clicado, chamando _pickAndCropProfileImage');
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
            labelText: 'Nome',
            hintText: _isBand == true ? 'Nome da banda' : 'Seu nome',
        ...
        TextFormField(
          ...
          decoration: InputDecoration(
            labelText: _isBand == true ? 'Ano de formação' : 'Ano de nascimento',
            hintText: _isBand == true
            ...
            prefixIcon: Icon(_isBand == true ? Icons.calendar_today : Icons.cake),
          ),
          validator: (value) {
  ...
  Widget _buildSkillsBlock() {
    return Column(
      ...
      children: [
        ...
        // Instrumentos
        MultiSelectField(
          title: _isBand == true ? 'Instrumentação' : 'Instrumentos',
