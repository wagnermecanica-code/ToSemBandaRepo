...
import 'package:wegig/features/messages/presentation/pages/chat_detail_page.dart';
import 'package:wegig/pages/post_detail_page.dart';
import 'package:wegig/pages/post_page.dart';
import '../widgets/profile_switcher_bottom_sheet.dart';
import 'package:image_cropper/image_cropper.dart';
import 'package:flutter_image_compress/flutter_image_compress.dart';
import 'package:path/path.dart' as p;
import '../../../../core/utils/deep_link_generator.dart';
...
class _ViewProfilePageState extends ConsumerState<ViewProfilePage>
  ...
  void initState() {
    super.initState();
    _tabController = TabController(length: 4, vsync: this); // 4 tabs: Gallery, YouTube, Posts, Interests
  ...
  Future<void> _loadProfileFromFirestore() async {
    ...
    try {
      ...
      if (widget.profileId != null) {
        ...
        debugPrint('📌 ViewProfilePage: Carregando perfil específico: $profileId');
      } else if (widget.userId == null || widget.userId == user.uid) {
        // Meu perfil ativo - utiliza cache atual e só força refresh se necessário
        debugPrint('👤 ViewProfilePage: Buscando perfil ativo do Riverpod sem forçar refresh');
        var profileState = ref.read(profileProvider);
        profile = profileState.value?.activeProfile;
        if (profile == null) {
          debugPrint('⚠️ ViewProfilePage: activeProfile null, forçando refresh único');
        ...
        debugPrint('✅ ViewProfilePage: Carregando MEU perfil ativo: $profileId');
      } else {
        ...
        debugPrint('ViewProfilePage: Carregando perfil de outro usuário: $profileId');
      }
      ...
      final doc = await FirebaseFirestore.instance.collection('profiles').doc(profileId).get();
      
      if (!doc.exists) {
        debugPrint('ViewProfilePage: Perfil $profileId não existe no Firestore');
        if (mounted) setState(() => _loadingProfile = false);
        return;
      }
      
      profile = Profile.fromMap(doc.data()!, doc.id);
      debugPrint('ViewProfilePage: Perfil carregado: ${profile.name} (${profile.isBand ? 'Banda' : 'Músico'})');
      
      // Buscar galeria do perfil (armazenada no documento do perfil)
      final gallery = (doc.data()?['gallery'] as List?)?.cast<String>() ?? [];
      ...
      debugPrint('✅ ViewProfilePage: Perfil carregado com sucesso - ${profile.name} (${profile.profileId})');
    } catch (e) {
  ...
  void didUpdateWidget(ViewProfilePage oldWidget) {
    ...
    if (oldWidget.userId != widget.userId || oldWidget.profileId != widget.profileId) {
      debugPrint('🔄 ViewProfilePage: didUpdateWidget detectou mudança nos parâmetros');
      _loadProfileFromFirestore();
    }
  }
  ...
  Future<void> _openOrCreateConversation() async {
    ...
    try {
      ...
      for (final doc in existingConversations.docs) {
        final participantProfiles = (doc.data()['participantProfiles'] as List?)?.cast<String>() ?? [];
      ...
      if (conversationId == null) {
        ...
        debugPrint('Criando conversa: currentUser.uid=${currentUser.uid}, otherUserId=$otherUserId');
        debugPrint('Participantes perfis: ${activeProfile.profileId}, ${_profile!.profileId}');
  ...
  Future<void> _shareProfile() async {
    ...
    try {
      ...
      final message = DeepLinkGenerator.generateProfileShareMessage(
        ...
        instruments: _profile!.instruments,
  ...
  Widget _buildGalleryImage(String pathOrUrl) {
    ...
    String candidate = pathOrUrl.startsWith('file://') 
        ...
        return _PhotoViewerPage(
          ...
          onDownload: (url) async {
            try {
              ...
              final fileName = 'download_${DateTime.now().millisecondsSinceEpoch}.jpg';
          ...
          onEdit: (url) async {
            ...
            try {
              ...
              debugPrint('ViewProfile: Nova imagem selecionada: ${picked.path}');
          ...
          onDelete: (url) async {
            try {
              ...
              final newGallery = _gallery.where((photo) => photo != url).toList();
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AsyncValue<ProfileState?>>(
      ...
      (previous, next) {
        ...
        final isViewingMyProfile = (widget.userId == null || widget.userId == FirebaseAuth.instance.currentUser?.uid) 
        ...
        if (previousProfileId != null && 
          ...
          debugPrint('🔄 ViewProfilePage: Perfil ativo mudou de $previousProfileId para $currentProfileId');
    ...
    return Scaffold(
                  ...
                  onPressed: () {
                    ProfileSwitcherBottomSheet.show(
                      ...
                      onProfileSelected: (newProfileId) async {
                        debugPrint('🔄 ViewProfile: Callback recebido - perfil trocado para $newProfileId');
                        // Aguarda um frame para garantir que o Riverpod atualizou
                        await Future.delayed(Duration(milliseconds: 100));
                        // Força reload imediato após troca
                        if (mounted) {
                          debugPrint('🔄 ViewProfile: Iniciando reload do perfil...');
                          await _loadProfileFromFirestore();
      ...
      body: _loadingProfile
          ...
          : _profile == null
              ? Center(child: Text('Perfil não encontrado'))
              : RefreshIndicator(
                        ...
                        children: [
                          ...
                          Hero(
                            ...
                            child: CircleAvatar(
                              ...
                              foregroundImage: _profile!.photoUrl != null && _profile!.photoUrl!.startsWith('http')
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  _profile!.name,
                                  style: TextStyle(fontWeight: FontWeight.w600, fontSize: 16.8),
                                  maxLines: 2,
  ...
  bool _hasSocialLinks() {
    return (_profile?.instagramLink != null && _profile!.instagramLink!.isNotEmpty) ||
  ...
  Widget _buildSocialLinksBlock() {
    return Container(
      ...
      child: Wrap(
        ...
        runSpacing: 8,
        children: [
          if (_profile?.instagramLink != null && _profile!.instagramLink!.isNotEmpty)
            _buildSocialIcon(
          ...
          if (_profile?.youtubeLink != null && _profile!.youtubeLink!.isNotEmpty)
            _buildSocialIcon(
  ...
  bool _shouldShowProfileInfo() {
    ...
    return (_profile!.level != null && _profile!.level!.isNotEmpty) ||
           _profile!.instruments.isNotEmpty ||
           _profile!.genres.isNotEmpty ||
           (_profile!.isBand && _profile!.bandMembers.isNotEmpty);
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (!_profile!.isBand && _profile!.level != null && _profile!.level!.isNotEmpty)
            Padding(
              padding: const EdgeInsets.only(bottom: 12),
              child: Row(
                children: [
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    spacing: 6,
                    runSpacing: 6,
                    children: _profile!.instruments.map((instrument) {
                      return Container(
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Icon(Icons.library_music, size: 18, color: Colors.grey[600]),
                      SizedBox(width: 8),
                      Text(
                        'Gêneros:',
                        style: TextStyle(
                  ...
                  Wrap(
                    spacing: 6,
                    runSpacing: 6,
                    children: _profile!.genres.map((genre) {
                      return Container(
                        padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                        decoration: BoxDecoration(
                          color: Colors.grey[200],
          ...
          if (_profile!.isBand && _profile!.bandMembers.isNotEmpty)
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Row(
                ...
                SizedBox(height: 6),
                Text(
                  '${_profile!.bandMembers.length} membros cadastrados',
  ...
  Widget _buildGalleryTab() {
    ...
    int totalItems = _gallery.length;
    ...
    if (_youtubeController == null) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.smart_display_outlined, size: 64, color: Colors.grey[400]),
            SizedBox(height: 16),
            Text(
  ...
  Widget _buildPostsTab() {
    ...
    final profileState = ref.read(profileProvider);
    final activeProfile = profileState.value?.activeProfile;
    final isOwner = (activeProfile != null && activeProfile.profileId == _profile!.profileId);
    ...
    return FutureBuilder<QuerySnapshot<Map<String, dynamic>>>(
      ...
      builder: (context, snap) {
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 16),
                Text('Erro ao carregar posts', style: TextStyle(color: Colors.grey[600])),
              ],
            ),
          );
        }
        ...
        return ListView.builder(
          shrinkWrap: true, // ✅ Permite ListView dentro de SingleChildScrollView
      ...
      builder: (context, snap) {
          ...
          return Center(
            child: Column(
              ...
              children: [
                Icon(Icons.error_outline, size: 64, color: Colors.red[300]),
                SizedBox(height: 16),
                Text('Erro ao carregar interesses', style: TextStyle(color: Colors.grey[600])),
              ],
            ),
          );
        }
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, i) {
            ...
            if (postId == null) return SizedBox.shrink();
            
            return FutureBuilder<DocumentSnapshot<Map<String, dynamic>>>(
              future: FirebaseFirestore.instance.collection('posts').doc(postId).get(),
              builder: (context, postSnap) {
                if (!postSnap.hasData || !postSnap.data!.exists) {
                  debugPrint('ViewProfile: Post $postId não encontrado ou deletado');
                  return SizedBox.shrink();
                }
                final postData = postSnap.data!.data()!;
                debugPrint('ViewProfile: Mostrando post $postId - type: ${postData['type']}');
                return _buildPostCard(postId, postData, false, isInterestCard: true);
              },
            );
          },
        );
  ...
  Future<QuerySnapshot<Map<String, dynamic>>> _fetchInterests() async {
    try {
      ...
      final activeProfile = profileState.value?.activeProfile;
      if (activeProfile == null) {
        debugPrint('ViewProfile: Perfil ativo não encontrado para buscar interesses');
        throw Exception('Perfil ativo não encontrado');
      }

      debugPrint('ViewProfile: Buscando interesses para profileId: ${activeProfile.profileId}');
  ...
  Future<void> _sendInterestOptimistically(String postId, String authorProfileId, String postType) async {
    if (!mounted) return;
  ...
  Future<void> _removeInterestOptimistically(String postId, String authorProfileId) async {
    if (!mounted) return;
    setState(() => _sentInterests.remove(postId));
  ...
  void _showInterestOptionsDialog(String postId, Map<String, dynamic> data, bool isInterestSent, bool isOwner) {
                ...
                onTap: () async {
                  ...
                  final confirmed = await showDialog<bool>(
                    ...
                    builder: (dialogCtx) => AlertDialog(
                      title: Text('Deletar Post'),
                      content: Text('Tem certeza que deseja deletar este post? Esta ação não pode ser desfeita.'),
                      actions: [
                        TextButton(
                          ...
                          SnackBar(
                            content: Row(
                              children: [
                                const Icon(Icons.error, color: Colors.white),
                                const SizedBox(width: 12),
                                Expanded(child: Text('Erro ao deletar post: $e')),
                              ],
                            ),
                            backgroundColor: Colors.red,
                            duration: const Duration(seconds: 4),
  ...
  Widget _buildPostCard(String postId, Map<String, dynamic> data, bool isOwner, {bool isInterestCard = false}) {
                  ...
                  children: [
                    ...
                    if (isOwner && daysLeft != null) ...[
                      ...
                      Text(
                        ...
                        style: TextStyle(
                          fontSize: 11,
                          color: daysLeft == 'Expirado' ? Colors.red : Colors.grey[600],
                          fontWeight: FontWeight.w500,
                        ),
                      ),
                    ],
            ...
            if (isOwner)
              Positioned(
                ...
                child: GestureDetector(
                  onTap: () => _showInterestOptionsDialog(postId, data, isInterestSent, isOwner),
                  child: Icon(
              ...
              Positioned(
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Icon(
                      isInterestSent ? Icons.favorite_rounded : Icons.favorite_border_rounded,
                      size: 16,
            ...
            ? [
                PopupMenuButton<String>(
                  ...
                  onSelected: (value) async {
                    ...
                    Navigator.of(context).pop(); // Fechar viewer antes de executar ação
