...
class ProfileRemoteDataSourceImpl implements ProfileRemoteDataSource {
  ...
  Future<List<ProfileEntity>> getAllProfiles(String uid) async {
    ...
    debugPrint('✅ ProfileRemoteDataSource: Encontrados ${profiles.length} perfis');
    return profiles;
  }
  ...
  Future<ProfileEntity?> getProfileById(String profileId) async {
    ...
    debugPrint('✅ ProfileRemoteDataSource: Perfil encontrado - ${profile.name}');
    return profile;
  }
  ...
  Future<ProfileEntity?> getActiveProfile(String uid) async {
    ...
    final activeId = (userDoc.data() as Map<String, dynamic>?)?['activeProfileId'] as String?;
    ...
    if (!profileDoc.exists) {
      debugPrint('⚠️ ProfileRemoteDataSource: Perfil ativo não encontrado no Firestore');
      return null;
    }
    ...
    await _firestore.runTransaction((transaction) async {
      ...
      if (!userDoc.exists) {
        ...
        if (currentActiveId == null || currentActiveId.toString().isEmpty) {
          ...
          debugPrint('📝 ProfileRemoteDataSource: activeProfileId definido para primeiro perfil');
        }
      }
    });
  ...
  Future<void> switchActiveProfile(String uid, String newProfileId) async {
    debugPrint('🔄 ProfileRemoteDataSource: switchActiveProfile - new=$newProfileId');
  ...
  Future<bool> isProfileOwner(String profileId, String uid) async {
    debugPrint('🔍 ProfileRemoteDataSource: isProfileOwner - id=$profileId, uid=$uid');
