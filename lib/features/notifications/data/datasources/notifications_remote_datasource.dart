...
abstract class INotificationsRemoteDataSource {
  ...
  Stream<List<NotificationEntity>> watchNotifications(String profileId, int limit);
...
class NotificationsRemoteDataSource implements INotificationsRemoteDataSource {
  ...
  Future<List<NotificationEntity>> getNotifications({
    ...
    try {
      debugPrint('📝 NotificationsDataSource: getNotifications for profile $profileId');
      ...
      debugPrint('📝 NotificationsDataSource: Encontradas ${snapshot.docs.length} notificações');
  ...
  Future<NotificationEntity?> getNotificationById(String notificationId) async {
    try {
      debugPrint('📝 NotificationsDataSource: getNotificationById $notificationId');
      ...
      if (!doc.exists) {
        debugPrint('⚠️ NotificationsDataSource: Notificação $notificationId não encontrada');
        return null;
      }
  ...
  Future<void> markAllAsRead(String profileId) async {
    try {
      debugPrint('📝 NotificationsDataSource: markAllAsRead for profile $profileId');
      ...
      debugPrint('📝 NotificationsDataSource: Marcando ${snapshot.docs.length} notificações como lidas');
      ...
      await batch.commit();
      debugPrint('✅ NotificationsDataSource: Todas notificações marcadas como lidas');
    } catch (e) {
  ...
  @override
  Future<void> deleteNotification(String notificationId, String profileId) async {
    try {
      debugPrint('📝 NotificationsDataSource: deleteNotification $notificationId');
  ...
  Future<NotificationEntity> createNotification(NotificationEntity notification) async {
    try {
      debugPrint('📝 NotificationsDataSource: createNotification type=${notification.type.name}');
      ...
      debugPrint('✅ NotificationsDataSource: Notificação criada com ID ${docRef.id}');
  ...
  Future<int> getUnreadCount(String profileId) async {
    try {
      debugPrint('📝 NotificationsDataSource: getUnreadCount for profile $profileId');
  ...
  @override
  Stream<List<NotificationEntity>> watchNotifications(String profileId, int limit) {
    debugPrint('📝 NotificationsDataSource: watchNotifications (stream) para profile $profileId');
    ...
    return _firestore
        ...
        .snapshots()
        .map((snapshot) {
      debugPrint('📝 NotificationsDataSource: Stream emitiu ${snapshot.docs.length} notificações');
      return snapshot.docs
          .map((doc) => NotificationEntity.fromFirestore(doc))
          .toList();
    });
  ...
  @override
  Stream<int> watchUnreadCount(String profileId) {
    debugPrint('📝 NotificationsDataSource: watchUnreadCount (stream) para profile $profileId');
    ...
    return _firestore
        ...
        .map((snapshot) {
      final count = snapshot.docs.length;
      debugPrint('📝 NotificationsDataSource: Stream emitiu $count notificações não lidas');
      return count;
    });
  }
}
