...
class _NotificationSettingsPageState
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          // Status de permissão
          _buildPermissionCard(permissionStatus),
  ...
  Widget _buildPermissionCard(AsyncValue<AuthorizationStatus> permissionStatus) {
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            SwitchListTile(
              value: activeProfile.notificationRadiusEnabled,
              onChanged: (value) => _toggleProximityNotifications(value),
              title: const Text('Ativar notificações de proximidade'),
              activeColor: AppColors.accent,
              contentPadding: EdgeInsets.zero,
            ),
            if (activeProfile.notificationRadiusEnabled) ...[
              const Divider(),
              const SizedBox(height: 8),
              Text(
                'Raio: ${activeProfile.notificationRadius.toInt()} km',
              ...
              Slider(
                value: activeProfile.notificationRadius,
                min: 5,
                max: 100,
                divisions: 19,
                label: '${activeProfile.notificationRadius.toInt()} km',
                activeColor: AppColors.accent,
                onChanged: (value) => _updateNotificationRadius(value),
              ),
              const Text(
  ...
  Future<void> _sendTestNotification() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('✅ Notificação de teste enviada! Verifique a aba de notificações.'),
          backgroundColor: AppColors.success,
