  ...
  Navigator.of(context).push(
    MaterialPageRoute(
      builder: (context) => PostPage(
        postType: postData['type'] ?? 'musician',
...
class _PostPageState extends ConsumerState<PostPage> {
  ...
  void _loadExistingData() async {
    ...
    _messageController.text = data['content'] ?? '';
    _youtubeController.text = data['youtubeLink'] ?? '';
    _level = data['level'] ?? 'Intermediário';
    ...
    if (data['seekingMusicians'] is List) {
      _selectedInstruments.addAll((data['seekingMusicians'] as List).cast<String>());
    }
    ...
    if (data['availableFor'] is List) {
      _selectedAvailableFor.addAll((data['availableFor'] as List).cast<String>());
      ...
      try {
        ...
        if (doc.exists && doc.data()?['availableFor'] is List) {
          setState(() {
            _selectedAvailableFor.addAll((doc.data()!['availableFor'] as List).cast<String>());
          });
        }
      } catch (e) {
  ...
  void _onAddressSelected(Map<String, dynamic> suggestion) {
    ...
    if (lat != 0.0 && lon != 0.0) {
      ...
      setState(() {
        ...
        _locationController.text = parts.isNotEmpty ? parts.join(', ') : suggestion['display_name'] ?? '';
        _selectedCity = city;
        _selectedNeighborhood = neighbourhood.isNotEmpty ? neighbourhood : null;
        _selectedState = state.isNotEmpty ? state : null;
      });
  ...
  Future<void> _pickPhoto() async {
    try {
      ...
      final targetPath = '${tempDir.path}/${DateTime.now().millisecondsSinceEpoch}_post.jpg';
      ...
      if (compressed == null) {
        ...
        debugPrint('✅ PostPage: Imagem comprimida: ${(compressedSize / 1024).toStringAsFixed(2)} KB');
  ...
  Future<void> _publish() async {
    ...
    final profile = profileAsync is AsyncData
        ? profileAsync.value?.activeProfile
        : null;
    ...
    try {
      ...
      if (_photoLocalPath != null) {
        if (_photoLocalPath!.startsWith('http')) {
          ...
          if (file.existsSync()) {
            final postId = widget.existingPostData?['postId'] ?? const Uuid().v4();
      ...
      if (_postType == 'musician') {
        ...
        postData['seekingMusicians'] = _selectedInstruments.toList(); // Banda busca músicos
      }
      ...
      debugPrint('PostPage: seekingMusicians = ${postData['seekingMusicians']}');
      debugPrint('PostPage: location = ${postData['location']} (${postData['location'].runtimeType})');
      ...
      debugPrint('❌ PostPage: StackTrace: $stackTrace');
      
      if (mounted) {
        String errorMessage = 'Erro ao publicar: $e';
        
        // Mensagens específicas para erros conhecidos
        if (e.toString().contains('firebase_storage')) {
          if (e.toString().contains('400')) {
            errorMessage = 'Erro ao fazer upload da imagem. Verifique se a imagem é válida e tente novamente.';
          } else if (e.toString().contains('permission')) {
            errorMessage = 'Sem permissão para fazer upload. Verifique suas credenciais.';
          } else {
            errorMessage = 'Erro no Firebase Storage: ${e.toString().replaceAll('firebase_storage/', '')}';
          }
                          ...
                          TextFormField(
                            ...
                            decoration: InputDecoration(
                              ...
                              helperStyle: TextStyle(
                                color: _extractYouTubeVideoId(_youtubeController.text) != null
                                    ? Colors.green
                                    : Colors.red,
                                fontSize: 12,
                              ),
