...
class _PostDetailPageState extends ConsumerState<PostDetailPage> {
  ...
  Future<void> _loadInterestedUsers() async {
    ...
    try {
      ...
      debugPrint('📊 Encontrados ${interestsSnapshot.docs.length} documentos na collection interests');
      
      List<Map<String, dynamic>> users = [];
      
      // Para cada interesse, buscar dados do perfil
      for (var interestDoc in interestsSnapshot.docs) {
      ...
      if (e.toString().contains('requires an index')) {
        debugPrint('⚠️ Índice do Firestore ainda não está ativo. Aguarde 2-5 minutos após o deploy.');
      }
  ...
  Future<void> _checkInterest(Post post) async {
    try {
  ...
  Future<void> _loadAuthorProfile(String profileId) async {
    try {
      ...
      if (profileDoc.exists) {
        final profileData = profileDoc.data()!;
        setState(() {
          _authorName = profileData['name'] ?? 'Perfil';
          _authorPhotoUrl = profileData['photoUrl'] ?? '';
        });
      }
    } catch (e) {
      debugPrint('Erro ao carregar perfil do autor: \$e');
    }
  }
  ...
  Future<void> _showInterest() async {
    ...
    try {
      ...
      await _loadInterestedUsers();
      debugPrint('✅ Interessados recarregados - _interestedUsers.length: ${_interestedUsers.length}');
  ...
  void _sharePost() {
    ...
    final text = DeepLinkGenerator.generatePostShareMessage(
      ...
      instruments: _post!.type == 'musician' ? _post!.instruments : _post!.seekingMusicians,
      genres: _post!.genres,
    );
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            SizedBox(
              width: _interestedUsers.length == 1 ? 32 : (_interestedUsers.length == 2 ? 52 : 72),
              height: 32,
              child: Stack(
                children: [
                ...
                text: TextSpan(
                  ...
                  children: [
                    ...
                    if (_interestedUsers.length == 2)
                      ...
                      TextSpan(
                        text: ' e outras ${_interestedUsers.length - 1} pessoas',
                        style: const TextStyle(fontWeight: FontWeight.w600),
                      ),
                  ],
                ),
                    ...
                    builder: (_) => PostPage(
                      ...
                      existingPostData: {
                        ...
                        'location': _post!.location != null
                            ? GeoPoint(_post!.location!.latitude, _post!.location!.longitude)
                            : null,
                      },
                    ),
                  ),
              ...
              children: [
                ...
                Hero(
                  ...
                  child: Container(
                    ...
                    child: (_post!.photoUrl != null && _post!.photoUrl!.isNotEmpty)
                        ? CachedNetworkImage(
                            ...
                            errorWidget: (context, url, error) => Container(
                              color: Colors.grey[300],
                              child: Icon(
                                _post!.type == 'band' ? Icons.group : Icons.person,
                                size: 80,
                                color: Colors.grey[600],
                              ),
                            ),
                              ...
                              GestureDetector(
                                ...
                                child: CircleAvatar(
                                  ...
                                  backgroundImage: _authorPhotoUrl.isNotEmpty
                                      ? CachedNetworkImageProvider(_authorPhotoUrl)
                                      : null,
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: _isOwnPost()
                              ...
                              : IconButton(
                                  icon: Icon(
                                    _hasInterest ? Icons.favorite : Icons.favorite_border,
                                    color: _hasInterest ? Colors.red : Colors.white,
                                    size: 18,
                                  ),
                                  padding: EdgeInsets.all(8),
                                  constraints: BoxConstraints(),
