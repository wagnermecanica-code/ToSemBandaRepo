...
class _EditPostPageState extends State<EditPostPage> {
  ...
  Future<void> _loadPostData() async {
        ...
        if (activeProfileId == user.uid) {
          ...
          if (profilesList != null) {
            try {
              ...
              _name = activeProfile['name'] ?? '';
              _profilePhotoUrl = activeProfile['photoUrl'];
            } catch (_) {
              _name = data['name'] ?? '';
              _profilePhotoUrl = data['photoUrl'];
            }
    ...
    setState(() {
      ...
      _selectedInstruments = (post['instruments'] as List<dynamic>?)?.map((e) => e.toString()).toSet() ?? <String>{};
      _selectedGenres = (post['genres'] as List<dynamic>?)?.map((e) => e.toString()).toSet() ?? <String>{};
      _seekingMusicians = (post['seekingMusicians'] as List<dynamic>?)?.map((e) => e.toString()).toSet() ?? <String>{};
      ...
      // Extract location if available and build display name for search field
      ...
      if (loc is GeoPoint) {
        ...
        // Build display string for location search field
        final parts = <String>[];
        if (_fetchedNeighborhood != null && _fetchedNeighborhood!.isNotEmpty) {
          parts.add(_fetchedNeighborhood!);
        }
        if (_fetchedCity != null && _fetchedCity!.isNotEmpty) {
          parts.add(_fetchedCity!);
        }
        if (_fetchedState != null && _fetchedState!.isNotEmpty) {
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, idx) {
                          ...
                          return CheckboxListTile(
                            value: selected,
                            title: Text(item),
                            onChanged: (v) => setStateDialog(()=> v! ? tempSelected.add(item) : tempSelected.remove(item)),
                          );
                        },
                      ),
                    ),
  ...
  Future<String?> _pickCropCompressAndGetPath() async {
    ...
    try {
      final XFile? picked = await _picker.pickImage(source: ImageSource.gallery, imageQuality: 95, maxWidth: 2000, maxHeight: 2000);
      if (picked == null) return null;

      final cropped = await ImageCropper().cropImage(
        sourcePath: picked.path,
        uiSettings: [
          AndroidUiSettings(toolbarTitle: 'Cortar imagem', toolbarColor: primaryColor, toolbarWidgetColor: Colors.white),
      ...
      final compressed = await FlutterImageCompress.compressAndGetFile(path, targetPath, quality: 85, minWidth: 800, minHeight: 800);
  ...
  Future<void> _updatePost() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('posts').doc(postId).update({
        'type': _postType,
        'seekingMusicians': _postType == 'band' ? _seekingMusicians.toList() : <String>[],
        ...
        'location': _selectedLocation != null
            ...
            : (widget.postData['location'] is GeoPoint ? widget.postData['location'] : FieldValue.delete()),
                                  ...
                                  children: [
                                    ...
                                    ListTile(
                                      ...
                                      leading: Radio<String>(
                                        value: 'musician',
                                        groupValue: _seeking, // groupValue ainda é o padrão correto para Radio
                                        onChanged: (v) => setState(() => _seeking = (_seeking == v ? null : v)),
                                        activeColor: AppThemeData.primaryColor,
                                      ),
                                      title: const Text('Músico'),
                                      subtitle: const Text('Procuro um músico para tocar junto'),
                                    ...
                                    ListTile(
                                      ...
                                      subtitle: const Text('Procuro uma banda para me juntar'),
                          ...
                          if (value.length >= 3) {
                            ...
                            setState(() {
                              ...
                              _showLocationSuggestions = false;
                                          ...
                                          itemBuilder: (context, index) {
                                            ...
                                            return ListTile(
                                              ...
                                              onTap: () {
                                                ...
                                                final neighbourhood = suggestion['neighbourhood'];
                                                ...
                                                setState(() {
                                                  ...
                                                  _fetchedCity = city;
                                                  _fetchedNeighborhood = neighbourhood;
                                                  _fetchedState = state;
                                                  _locationValidated = true;
                                                });
                                                
                                                debugPrint('EditPostPage: localização selecionada: $city, lat=$lat, lng=$lon');
                                              },
                                            );
                                          },
                                        ),
                                    ...
                                    if (_locationValidated && (_fetchedCity != null || _fetchedNeighborhood != null))
                                      Padding(
                                        ...
                                        child: Container(
                                          ...
                                          child: Row(
                                            children: [
                                              ...
                                              Expanded(
                                                child: Text(
                                                  '${_fetchedNeighborhood ?? ''}${(_fetchedNeighborhood != null && _fetchedNeighborhood!.isNotEmpty && _fetchedCity != null && _fetchedCity!.isNotEmpty) ? ', ' : ''}${_fetchedCity ?? ''} - ${_fetchedState ?? ''}',
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    TextFormField(
                                      ...
                                      onChanged: (_) => setState(() {}), // Recarrega para mostrar preview
                                    ),
                ...
                child: SafeArea(
                  ...
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      ...
                      disabledBackgroundColor: AppThemeData.primaryColor.withValues(alpha: 0.5),
